...
| 服务 | 容器名 | 宿主机端口 | 用途 |
|---|---|---:|---|
| Frontend + Nginx | `hellohebut-frontend` | `80`，可通过 `FRONTEND_PORT` 修改 | Vue 前端及 API 反向代理 |
| 报考咨询 API | `hellohebut-app` | `8000` | FastAPI 后端 |
| ChromaDB | `hellohebut-chromadb` | `8001` | RAG、情景记忆和用户画像的向量存储 |
| Redis | `hellohebut-redis` | `6379` | 工作记忆和会话缓存 |
| Prometheus | `hellohebut-prometheus` | `9090` | 指标采集和监控 |
...
```powershell
docker compose up -d --force-recreate hellohebut
```
...
```powershell
docker compose restart hellohebut
docker compose logs -f hellohebut
```
